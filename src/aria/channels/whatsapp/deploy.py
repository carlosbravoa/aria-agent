"""
aria/channels/whatsapp/deploy.py — Deploy the Node WhatsApp bridge files.
...
def source_dir() -> Path | None:
    ...
    # src/aria/channels/whatsapp/deploy.py → parents[4] == repo root
    candidates.append(Path(__file__).resolve().parents[4] / "whatsapp")
    for c in candidates:
