...
def test_shell_run_marked_interactive(minimal_env):
    """shell_run may block on a confirm prompt, so it opts into INTERACTIVE and
    the agent lists it in `_interactive` (which suppresses the live spinner that
    would otherwise clobber the prompt)."""
    from aria.agent import Agent
    from aria.tools import shell_run
    assert shell_run.INTERACTIVE is True
    a = Agent()
    assert "shell_run" in a._interactive
    # a read-only tool must NOT be interactive
    assert "web_fetch" not in a._interactive
