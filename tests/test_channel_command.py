...
def test_repl_channel_command(systemd, monkeypatch):
    ...
    assert "fake" in buf.getvalue() and "offline" in buf.getvalue()
    main._channel_command("on fake --always")        # fake can't attach → background
    assert "aria-channel-fake: active" in buf.getvalue()
    assert "fake is online in the background" in buf.getvalue()
    main._channel_command("bogus")
    assert "Usage" in buf.getvalue()
    main._channel_command("on nope")
